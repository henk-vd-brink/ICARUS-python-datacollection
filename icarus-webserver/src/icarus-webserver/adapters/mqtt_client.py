...
class MqttClient(paho.Client):
    ...
    @classmethod
    def build_client_from_config(cls, config, sleep_time=3):
        client = MqttClient(config=config)
        CONNECTED = False

        while not CONNECTED:
            try:
                client.connect(config.broker_ip_address, config.broker_port)
                CONNECTED = True
            except Exception:
                time.sleep(sleep_time)
                logging.info(
                    f"Could not connect to MQTT broker. Try again in {sleep_time}s."
                )
        return client
